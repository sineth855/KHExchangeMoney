<template>
  <aside class="aside-menu">
    <b-tabs>
      ...
      <b-tab title="<i class='icon-bell'></i>">
        <!--{{dataNotification | json}}-->
        <div class="p-3" v-for="data, key in dataNotification['data']">
          <!--row notification-->
          <div class="message">
            ...
            <div>
              <small class="text-muted">Account No (<a href="javascript:void(0)">{{data.account_no}}</a>)</small>
              <!--<small class="text-muted float-right mt-1">1:52 PM</small>-->
            </div>
            <div class="text-truncate font-weight-bold">{{data.notification_title}}</div>
            <small class="text-muted">{{data.transaction_detail}}</small>
            <hr/>
          </div>
          <!--row notification-->
          <!--<div class="message" v-for="let data of dataNotification">
            <div class="py-3 pb-5 mr-3 float-left">
              <div class="avatar">
                <img src="static/img/avatars/7.jpg" class="img-avatar" alt="admin@bootstrapmaster.com">
                <span class="avatar-status badge-success"></span>
            ...
            <div>
              <small class="text-muted">SINETH SIM (002344)</small>
              <small class="text-muted float-right mt-1">1:52 PM</small>
            </div>
            <div class="text-truncate font-weight-bold">Transfer Money</div>
            <small class="text-muted">Transfer money to my sister at takeo.</small>
            <hr>
          </div>-->
  ...
  methods: {
    getNotification() {
      fetchList().then(response => {
        this.dataNotification = response
      })
    },
  }
}
